...
export default async function InsightsPage({ searchParams }: { searchParams:Promise<{ range?:string;from?:string;to?:string }> }) {
  ...
  const [{ count:lifetimeViews },{ count:lifetimeClicks },{ count:periodViews },{ count:periodClicks },{ data:viewEvents },{ data:clickEvents },{ data:links }]=await Promise.all([
  ...
  return <InsightsDashboard locale={locale} period={period} totals={{views:periodViews??0,clicks:periodClicks??0,lifetimeViews:lifetimeViews??0,lifetimeClicks:lifetimeClicks??0}} days={days} clickedLinks={clickedLinks}/>;
}