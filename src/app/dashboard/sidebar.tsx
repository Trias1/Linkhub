...
export function DashboardSidebar({name,email,slug,isAdmin,activeId,linktrees,locale}:{name:string;email:string;slug?:string|null;isAdmin:boolean;activeId:string;linktrees:Linktree[];locale:Locale}){
  const pathname=withoutLocale(usePathname());const [collapsed,setCollapsed]=useState(false);const en=locale==="en";
  useEffect(()=>{const media=window.matchMedia("(max-width:1100px)");const reset=()=>{if(media.matches)setCollapsed(false)};reset();media.addEventListener("change",reset);return()=>media.removeEventListener("change",reset)},[]);
  return <aside className={`dashboard-sidebar${collapsed?" collapsed":""}`}>
    <div className="sidebar-top"><button className="brand brand-toggle" onClick={()=>setCollapsed((value)=>!value)} type="button" aria-label={collapsed?(en?"Open sidebar":"Buka sidebar"):(en?"Close sidebar":"Tutup sidebar")}>{collapsed?"LH":"LinkHub"}</button></div>
